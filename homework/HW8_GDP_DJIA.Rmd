...
```{r}
cor_gdp_djia<- gdp_djia |> 
  summarize(correlation = cor(GDP, DJIA))
print(correlation)
```
...
gdp_djia |> 
 ...
 geom_text(aes(x = GDP, y = DJIA, label = Quarter), size = 3,
            nudge_x = 0, nudge_y = 2500, angle = 90)
```
...
The scatterplot seems to show that when both the DJIA and the GDP are low, there is a much stronger correlation between the two than when they both increase. However, with that being said, the correlation is only .58. So there may be some sort of positive correlation between the two, however it is not strong.
