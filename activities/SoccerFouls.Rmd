...
```{r}
soccer_diff |> 
  summarize(correlation = cor(fouls, goal_diff))
```
...
With the correlation score being 0.018 away from zero, it is pretty safe to say that there is no correlation between goals and fouls. This is also backed up by the scatterplot that has no pattern besides a large blob in the middle. 
